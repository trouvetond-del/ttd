...
export default function MoverPortfolioPage() {
  const { user } = useAuth();
  ...
  return (
    <MoverLayout title="Mon Portfolio">
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-6">Mon Portfolio</h2>
          <PortfolioGallery moverId={moverId} isOwner={true} />
        </div>
      )}
    </MoverLayout>
  );
